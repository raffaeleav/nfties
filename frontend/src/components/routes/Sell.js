...
import { SellGrid } from '../SellGrid.js';
...
export const Sell = () => {
    ...
    const fetchIpfsMetadata = async (cid) => {
        if (!cid) {
            console.log("[error] (Sell.js:fetchIpfsMetadata) cid is required");
        ...
        try {
            ...
            console.log("[error] (Sell.js:fetchIpfsMetadata) " + error);
            
            return null;
        }
    };

    const getSellRouteNFTs = useCallback(async () => {
        ...
        if (token) {
            ...
            console.log('[log] (Sell.js:getSellRouteNFTs) user ' + walletAddress + ' tried to mintNFT with wrong jwt');
            navigate('/');
        ...
        try {
            ...
            for (let i = 0; i < mintedTokens; i++) {
                try {
                    ...
                    console.log('[log] (Sell.js:getSellRouteNFTs) token #' + i + ' is not listed'); 
                }
            }
            ...
            if (tokenIds.length > 0) {
                for (let i = 0; i < mintedTokens; i++) {
                    if (listings[i].seller.toLowerCase() === walletAddress.toLowerCase()) {
                        ...
                        if (!metadataJSON) {
                            console.log('[log] (Sell.js:getSellRouteNFTs) no metadata found for token ' + (i + 1));
                        }
                ...
                console.log('[log] (Sell.js:getSellRouteNFTs) no nft found, showing 0 search results'); 
            }
            ...
            console.log('[error] (Sell.js:getSellRouteNFTs) ' + error);
    ...
    useEffect(() => {
        const fetchNFTs = async () => {
            const fetchedNFTs = await getSellRouteNFTs();
            setNfts(fetchedNFTs);
        };
            ...
            return (
                <div>
                    <div className="search-results">
                        <h2>Sell<span className="subtitle"> your NFTs</span></h2>
                    </div>
                    <SellGrid nfts={nfts} />
                </div>
            )
